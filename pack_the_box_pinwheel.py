""" Pack the box. Pinwheel version.

Container is divided into quadrants.
Each attempt to pack the container rotates through all four quadrants, repeats.

Placed items track quadrant: placed_item = {'item': shape, 'quadrant': quadrant, 'coordinates': {'x': x, 'y': y}}

The idea here is that by aiding the equal spread of items in the container fewer failed attempts to pack will occur.
...
def pack_item(container,shape,limits,quadrant):

    x,y = gen_random_coord(limits,quadrant)

    if does_not_overlap(container,shape,x,y):
        placed_item = {'item': shape, 'quadrant': quadrant, 'coordinates': {'x': x, 'y': y}}
...
def gen_random_coord(limits,quadrant):
    ...
    x = round(uniform(0.,limit_x), 2)
    y = round(uniform(0.,limit_y), 2)
